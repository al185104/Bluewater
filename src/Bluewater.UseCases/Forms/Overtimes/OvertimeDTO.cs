...
public record OvertimeDTO() {
    ...
    public OvertimeDTO(Guid id, Guid empId, string? name, DateTime? startDate, DateTime? endDate, int? approvedHours, string? remarks, ApplicationStatusDTO status) : this()
    {
        Id = id;
        EmpId = empId;
        Name = name;
