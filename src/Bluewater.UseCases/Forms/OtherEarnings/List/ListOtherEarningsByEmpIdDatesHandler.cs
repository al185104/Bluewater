...
internal class ListOtherEarningsByEmpIdDatesHandler(IRepository<OtherEarning> _repository) : IQueryHandler<ListOtherEarningsByEmpIdDatesQuery, Result<IEnumerable<OtherEarningDTO>>>
{
  public async Task<Result<IEnumerable<OtherEarningDTO>>> Handle(ListOtherEarningsByEmpIdDatesQuery request, CancellationToken cancellationToken)
  {
    var spec = new OtherEarningByEmpIdDatesSpec(request.empId, request.start, request.end);
