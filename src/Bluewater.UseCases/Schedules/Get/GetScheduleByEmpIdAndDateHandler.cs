...
public class GetScheduleByEmpIdAndDateHandler(IRepository<Schedule> _schedRepository, IRepository<Employee> _empRepository) : IQueryHandler<GetScheduleByEmpIdAndDateQuery, Result<ScheduleDTO>>
{
  public async Task<Result<ScheduleDTO>> Handle(GetScheduleByEmpIdAndDateQuery request, CancellationToken cancellationToken)
  {
    var spec = new ScheduleByEmpIdAndDateSpec(request.EmpId, request.EntryDate);
