  ...
  public async Task<Result<IEnumerable<AttendanceDTO>>> Handle(ListAttendanceQuery request, CancellationToken cancellationToken)
  {
    ...
    for (var date = request.startDate; date <= request.endDate; date = date.AddDays(1))
    {
      ...
      if(attendance == null) {
        ...
        {
          timesheet = ApprovedLeaveWorkHoursCalculator.CreateSyntheticTimesheet(emp.Id, date, schedule?.Shift);
        }
        ...
        if (leaveId.HasValue)
        {
          if (!leaveStatuses.TryGetValue(leaveId.Value, out leaveStatus) && approvedLeave is not null)
          {
