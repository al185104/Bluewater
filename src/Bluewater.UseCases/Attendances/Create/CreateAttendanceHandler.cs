...
public class CreateAttendanceHandler(IRepository<Attendance> _repository) : ICommandHandler<CreateAttendanceCommand, Result<Guid>>
{
  ...
  {
    var newAttendance = new Attendance(request.EmployeeId, request.ShiftId, request.TimesheetId, request.LeaveId, request.EntryDate, request.WorkHrs, request.LateHrs, request.UnderHrs, request.OverbreakHrs, request.NightShiftHrs, request.IsLocked);
